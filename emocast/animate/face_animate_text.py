...
from emocast.models.mutual_self_attention_text import ReferenceAttentionControl
...
class FaceAnimatePipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        inputs = self.clip_tokenizer([emo_label], max_length=self.clip_tokenizer.model_max_length,padding='max_length', return_tensors="pt").to(device=self.text_encoder.device)

        outputs = self.text_encoder(**inputs)

        emo_text_emb = outputs.last_hidden_state

        encoder_hidden_states=torch.cat((emo_text_emb,encoder_hidden_states),dim=1)
        

        uncond_encoder_hidden_states = self.image_proj(torch.zeros_like(clip_image_embeds))

        inputs_none=self.clip_tokenizer([""], max_length=self.clip_tokenizer.model_max_length,padding='max_length', return_tensors="pt").to(device=self.text_encoder.device)

        uncond_outputs = self.text_encoder(**inputs_none)

        uncond_emo_text_emb = uncond_outputs.last_hidden_state

        uncond_encoder_hidden_states=torch.cat((uncond_emo_text_emb,uncond_encoder_hidden_states),dim=1)
        ...
        #here new emo_label
        emo_text_embs=emo_text_emb.unsqueeze(1)
        emo_text_embs=emo_text_embs.expand(-1,audio_tensor.shape[1],-1,-1)


        emo_text_embs=rearrange(emo_text_embs,"b f t d -> (b f) t d")
        emo_text_embs_cat=torch.cat([emo_text_embs,emo_text_embs],dim=0)

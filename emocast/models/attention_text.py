    ...
    def __init__(
        ...
        if cross_attention_dim is not None or double_self_attention:
            ...
            self.attn2 = Attention(
                ...
                bias=attention_bias,
                upcast_attention=upcast_attention,
                processor=IPAdapterAttnProcessor2_0(hidden_size=num_attention_heads*attention_head_dim,cross_attention_dim=(
                    cross_attention_dim if not double_self_attention else dim)),
    ...
    def __init__(
        ...
        if cross_attention_dim is not None:
            self.attn2 = Attention(
                ...
                bias=attention_bias,
                upcast_attention=upcast_attention,
                processor=IPAdapterAttnProcessor2_0(hidden_size=num_attention_heads*attention_head_dim,
                                                    cross_attention_dim=cross_attention_dim), 
            )
        else:
            self.attn2 = None
...
class EmoCrossAudioTemporalBasicTransformerBlock(nn.Module):
    """
    ...
    def __init__(
        ...
        # SC-Attn
        self.attn1 = Attention(
            #dim=num_attention_heads * attention_head_dim(Transformer3DModel audio_module)
            query_dim=dim,
            heads=num_attention_heads,
            dim_head=attention_head_dim,
            dropout=dropout,
        ...
        # SC-Attn
        #audio
        #torch.Size([56, 32, 768])
        #emo text
        #torch.Size([56, 77, 768])
        #audio&text dim
        audio_dim=768
        audio_cross_attention_dim=768
        audio_attention_head_dim=80
        self.attn_audioemo = Attention(
            #dim=num_attention_heads * attention_head_dim(Transformer3DModel audio_module)
            query_dim=audio_dim,
        ...
        self.postnorm_audioemo = nn.LayerNorm(audio_dim)
    ...
    def forward(
        ...
        encoder_hidden_states=self.attn_audioemo(emo_label,encoder_hidden_states=encoder_hidden_states)

        encoder_hidden_states=self.postnorm_audioemo(encoder_hidden_states)

...
from .transformer_3d_text import Transformer3DModel
...
class CrossAttnDownBlock3D(nn.Module):
    ...
    def __init__(
        self,
        in_channels: int,#block_out_channels: Tuple[int] = (320, 640, 1280, 1280),
        out_channels: int,
        temb_channels: int,
        dropout: float = 0.0,
        num_layers: int = 1,
        ...
        resnet_groups: int = 32,
        resnet_pre_norm: bool = True,
        attn_num_head_channels=1,#attention_head_dim: Union[int, Tuple[int]] = 8,
        cross_attention_dim=1280,
        audio_attention_dim=1024,
        output_scale_factor=1.0,
        ...
        for i in range(num_layers):
            ...
            audio_modules.append(
                Transformer3DModel(
                    ...
                    #num_attention_heads (in Transformer3DModel audio_module)
                    in_channels // attn_num_head_channels,  # Tuple[int] = (40, 80, 160, 160),
                    #attention_head_dim  (in Transformer3DModel audio_module)
                    in_channels=out_channels,
                    num_layers=1,
                    cross_attention_dim=audio_attention_dim,
                    norm_num_groups=resnet_groups,

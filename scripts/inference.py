...
from emocast.animate.face_animate_text import FaceAnimatePipeline
from emocast.datasets.audio_processor import AudioProcessor
from emocast.datasets.image_processor import ImageProcessor
from emocast.models.audio_proj import AudioProjModel
from emocast.models.face_locator import FaceLocator
from emocast.models.image_proj import ImageProjModel
from emocast.models.unet_2d_condition_text import UNet2DConditionModel
from emocast.models.unet_3d_text import UNet3DConditionModel
from emocast.utils.config import filter_non_none
from emocast.utils.util import tensor_to_video
...
def inference_process(args: argparse.Namespace):
    ...
    if os.path.isfile(driving_audio_path):
        audio_emb, audio_length = audio_processor.preprocess(driving_audio_path, clip_length)
        ...
        audio_name = os.path.basename(driving_audio_path).split('.')[0]
        ref_name = os.path.basename(source_image_path).split('.')[0]
        output_file = os.path.join(outputs_dir,f"{ref_name}_{audio_name}.mp4")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--driving_audio", type=str, required=False,
                        help="driving audio")
    parser.add_argument("--prompt_emo", type=str, required=False,
                        help="prompt")
    # parser.add_argument(
    #     "--output", type=str, help="output video file name", default=".cache/output.mp4")
    parser.add_argument(
        "--outputs", type=str, help="output video file name", default="outputs")
    parser.add_argument(
